...
class Market_DB_Updater:
    ...
    def read_creon_data(self, idx, rate, date, A_code, company):
        ...
        try:
            ...
            dict_data = {'date': date, '시가': objPinfo.GetDataValue(0, 0), '종가': objPinfo.GetDataValue(1, 0),
                    ...
                    '은행': objTinfo.GetDataValue(7, 0), '연기금': objTinfo.GetDataValue(9, 0),
                    '사모펀드': objTinfo.GetDataValue(12, 0)}
        
        except Exception as e:
            print('Exception occured :', str(e))
            print(f'================Exception at {A_code} / {date}================')

            dict_data = {'date': date, '시가': 0, '종가': 0, '전일대비': 0, '거래대금': 0,
                    '시가총액': 0, '개인': 0, '외국인': 0, '기관': 0, '금융투자': 0,
                    '보험': 0, '투신': 0, '은행': 0, '연기금': 0, '사모펀드': 0}
        return dict_data


    def update_market_db(self, df_data, idx, code, company, get_date):
        """Update CreonData to market_db Table"""
        with self.conn.cursor() as curs:
            for r in df_data.itertuples():
                sql = f"REPLACE INTO market_db VALUES ('{code}', "\
                    f"'{r.date}', {r.시가}, {r.종가}, {0}, {r.전일대비}, {r.거래대금}, {r.시가총액}, "\
                    f"{r.개인}, {r.외국인}, {r.기관}, {r.금융투자}, {r.보험}, {r.투신}, {r.은행}, {r.연기금}, {r.사모펀드})"
                curs.execute(sql)
            self.conn.commit()
            print('[{}] #{:04d} {} ({}) > REPLACE INTO daily_trends [{}][OK]'.format(datetime.now().strftime('%Y-%m-%d %H:%M'),
                                                                                           idx+1, company, code, get_date))
    ...
    def convert_to_DataFrame(self, dict_data):
        ...
        df_data = df_data[['date', '시가', '종가', '전일대비', '거래대금', '시가총액', '개인',
    ...
    def get_rate(self, code, date):
        with self.conn.cursor() as curs:
            sql = f"SELECT * FROM `marketdb_rate`.`market_db_rate` WHERE code = '{code}' and date = '{date}"
            rate_df = pd.read_sql(sql, self.conn)
        return rate_df['등락률'][0]


    def execute_updater(self):
        """Excute the whole code"""
        self.update_comp_info()
        s_datepram = date(2016, 7, 27) # 조회 시작일
        e_datepram = date(2021, 1, 7) # 조회 종료일
        date_info = self.m_db.get_date_db('005930')['date']
        date_idx = len(date_info) - 1
        
        while(s_datepram <= e_datepram):
            if (date_info[date_idx] > e_datepram):
                date_idx -= 1
                continue
            elif (date_info[date_idx] < e_datepram):
                ...
                for idx, code in enumerate(self.codes):
                    ...
                    rate = self.get_rate(code, e_datepram)
                    dict_data = self.read_creon_data(idx, rate, cvt_date, 'A' + code, self.codes[code])
                    df_data = self.convert_to_DataFrame(dict_data)
                    if df_data is None:
                        continue
                    self.update_market_db(df_data, idx, code, self.codes[code], e_datepram)
